import {  modifiers, typesSet, directive_chars, directives, tt, reserved_words_set, modifiers_set } from "./defs";
...
export class MultiLineStream {
	...
	// 	const match = this.source.substring(this.position).match(/^[a-zA-Z][\w]*/);
	// 	if (match && modifiers[match[0]]) {
	...
	public advanceIfModifier(): string {
		return this.advanceIfWordWithTest(match => modifiers_set.has(match[0].toLowerCase()));
	}

	public advanceIfType(): string {
		return this.advanceIfWordWithTest(match => typesSet.has(match[0].toLowerCase()));
	}
	
	public advanceIfWordNonReserved(): string {
		return this.advanceIfWordWithTest(match => !reserved_words_set.has(match[0].toLowerCase()));		
	}
	...
	public advanceUntilUnescapedQuote(): boolean {
		while (this.position < this.source.length) {
			if (this.source.charCodeAt(this.position) === tt._DQO
	...
	public skipWhitespace(includeLineBreaks=true): boolean {
		const n = this.advanceWhileChar(ch => {
			return ch === tt._WSP || ch === tt._TAB || (includeLineBreaks && (ch === tt._CAR || ch === tt._NWL || ch === tt._LFD));
		});
		return n > 0;
	}
